...
def benchmark_naive(df, order_size=5000):
    """Naive strategy: fills as much as possible at each timestamp until order is complete."""
    remaining_qty = order_size
    total_cost = 0
    filled_shares = 0
    timestamps = np.sort(df['timestamp'].unique())
    ...
    for ts in timestamps:
        ...
        # Get best available price at this timestamp (lowest ask)
        current_venues = df[df['timestamp'] == ts]
        best_venue = current_venues.loc[current_venues['ask'].idxmin()]
        
        executed = min(remaining_qty, best_venue['ask_size'])
...
def benchmark_twap(df, order_size=5000):
    """TWAP strategy: splits order evenly across all timestamps."""
    timestamps = np.sort(df['timestamp'].unique())
    ...
    for ts in timestamps:
        ...
        executed = min(qty_per_step, best_venue['ask_size'])
        cost = executed * (best_venue['ask'] + best_venue['fee'])
...
def benchmark_vwap(df, order_size=5000):
    """VWAP strategy: allocates order proportionally to available volume."""
    timestamps = np.sort(df['timestamp'].unique())
    total_volume = df.groupby('timestamp')['ask_size'].sum().sum()
    if total_volume == 0:
        return {
            'strategy': 'vwap',
            'total_cost': 0,
    ...
    for ts in timestamps:
        current_venues = df[df['timestamp'] == ts]
        ts_volume = current_venues['ask_size'].sum()
        if ts_volume == 0:
            continue

        # Calculate target quantity based on volume proportion
        target_qty = (ts_volume / total_volume) * order_size
        remaining_in_ts = target_qty

        # Execute orders across venues (sorted by best price)
        current_venues = current_venues.sort_values('ask')
        for _, venue in current_venues.iterrows():
            if remaining_in_ts <= 0:
                break

            executed = min(remaining_in_ts, venue['ask_size'])
            cost = executed * (venue['ask'] + venue['fee'])
            
            filled_shares += executed
            total_cost += cost
            remaining_in_ts -= executed
